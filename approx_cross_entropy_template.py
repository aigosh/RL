...
def generate_session(agent: MLPClassifier, t_max=10**5):
    states, actions = [], []
    total_reward = 0
    ...
    for t in range(t_max):
        ...
        # a = ?
        probas = agent.predict_proba([s])
        a = np.random.choice(n_actions, p=probas[0])
...
def test_rl_approx_cross_entropy(nn_agent: MLPClassifier):
    s = env.reset()
    total_reward = 0
    for t in range(1000):
        ...
        # You can use np.random.choice() func
        # a = ?
        probas = nn_agent.predict_proba([s])
        a = np.random.choice(n_actions, p=probas[0])
...
if __name__ == '__main__':
    ...
    env: gym.Env = gym.make('CartPole-v0').env
    s = env.reset()
