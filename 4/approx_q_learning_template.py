...
if __name__ == '__main__':
    ...
    for i in range(max_epochs):
        ...
        if np.mean(session_rewards) - prev_mean > -1:
            epsilon = max(epsilon * np.log1p(epsilon / (1 - np.exp(-epsilon))), 1e-3)
        else:
            epsilon = min(epsilon / np.mean(session_rewards) * prev_mean, max_epsilon)

        prev_mean = np.mean(session_rewards)
